...
public class SolicitacaoPropostaRepositoryImpl extends AbstractRepositoryImpl<SolicitacaoProposta> implements SolicitacaoPropostaRepository {
     ...
     *
     * @param regiao a ser pesquisada
     *
     * @return retornar as solicitações por regiao.
     */
    @Override
    public List<Regiao> buscarSolicitacoesPorRegiao(Regiao regiao) {
        Query q = em.createQuery(" from SolicitacaoProposta obj "
                + " where cast( substring(obj.cep, 1 , 5) as long ) between :cepInicial and :cepFinal ");
        q.setParameter("cepInicial", regiao.transformaStringCepEmLong(regiao.getCepInicial()));
        q.setParameter("cepFinal", regiao.transformaStringCepEmLong(regiao.getCepFinal()));
